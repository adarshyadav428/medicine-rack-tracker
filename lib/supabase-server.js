...
function toCloudRow(item) {
  ...
  const sellingPrice = toDecimalOrNull(item.sellingPrice ?? item.sellPrice);
  const purchasePrice = toDecimalOrNull(item.purchasePrice ?? item.rate);
  ...
  return {
    ...
    rate: purchasePrice,
    discount: toDecimalOrNull(item.discount),

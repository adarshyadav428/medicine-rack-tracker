...
const allowedPages = new Set(["index.html", "dashboard.html", "access.html", "billing.html"]);
...
function enforcePageGuard() {
  ...
  if (currentPage === "billing") {
    if (!isCloudSyncActive() || !isAuthenticated()) {
      goTo("index.html?next=billing.html");
      return false;
    }
...
async function handleAuthSession(session, source = "session") {
  ...
  try {
    ...
    // Notify billing.js once auth is ready
    if (currentPage === "billing" && typeof window.__onBillingReady === "function") {
...
function renderPage() {
  ...
  renderDashboardPage();
  renderAccessPage();
  if (typeof window.__renderBillingPage === "function") {
    window.__renderBillingPage();
  }
}
